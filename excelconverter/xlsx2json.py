...
def convert(xlsxfile):
	...
	if ws.title == "ObjectList":
		...
		for row in rowsGenerator:
			...
			jsonObject.append(__parseDict(item))
	elif ws.title == "PrimitiveList":
		jsonObject = []
		for row in rowsGenerator:
			jsonObject.append(row[0].value)
	elif ws.title == "SingleObject":
		jsonObject = __parseDict(dict(zip(header, [cell.value for cell in rowsGenerator.next()])))
	else:
		raise Exception('Excel type not supported')
	with open(tempJsonFilename, 'w') as tempJsonFile:
		tempJsonFile.write(json.dumps(jsonObject, indent = 4))
